...
for t = 1:T
    ...
    [d_kj, d_bias] = ExistingNodes;  % pg 6, line 3
    [dn_k, un_k] = NewNodes; %pg 6, line 4
    [jk_best, e_t, numLayers, numNodes] = BestNode; %pg 6, line 5
...
%         figure(1), plot(adaParams.lossStore)
%         titleStr = 'numNodes = (';
...
%     end
    ...
    function [d_kj,d_bias] = ExistingNodes
        ...
        for k = 1:numLayers
            if ~cfg.javier
                ...
                Ck = Rloss(k)/4^(k-1); % per Javier's recommendation
            end
            % inference using existing parameters
            
            switch cfg.lossFunction
                case 'binary'
                    errLoss{k} = Ck/2 * (1-1/Ck*bsxfun(@times, ydata(lossIdcs,:),H{k}(lossIdcs,:))'*Dloss);                otherwise
                    error('loss not supported');
            end
        ...
        switch cfg.lossFunction
            case 'binary'
                errLoss_bias = Ck_bias/2 * (1-1/Ck_bias*bsxfun(@times, ydata(lossIdcs,:),ones(size(ydata(lossIdcs,:))))'*Dloss);            otherwise
                error('loss not supported');
        end
    ...
    function [dn_k, un_k] = NewNodes
        ...
        if cfg.javier
            ...
            big_lambdas = zeros(adaParams.maxLayers,1);
            for n = 0:adaParams.maxLayers-1
        ...
        for k = 1:idx        % pg 8, Fig 4, Line 1
            ...
            if ~cfg.javier
                ...
                Ck = Rnew(k)/4^(k-1); % per Javier's recommendation
                Lambda_k = big_lambdas(k);
              % adding per J.Gonzalva recommendation
            end
